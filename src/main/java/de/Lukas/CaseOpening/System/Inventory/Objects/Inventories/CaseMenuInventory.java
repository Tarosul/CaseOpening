...
public class CaseMenuInventory extends de.Lukas.CaseOpening.System.Inventory.Objects.CaseInventory {
    ...
    public void onOpenInventory(Player p) {
        ...
        credits = String.valueOf(Credits.getCredits(p.getUniqueId().toString()));
        ...
        for (int i = 0; i < CaseManager.getItems().size(); i++) {
            ...
            int amount = CaseAmount.getCases(p.getUniqueId().toString(), id);
    ...
    public void onClick(Player paramPlayer, ItemStack paramItemStack) {
        ...
        for (int caseSlot : cases.keySet()) {
            if (caseSlot == slot) {
                ...
                checkCase(paramPlayer, getClickEvent(), uuid, caseType, caseId, amount);
            }
        }

        switch (slot) {
            case 30 -> CaseOpening.getCaseInventory("cases").openInventory(paramPlayer);
            case 32 -> CaseOpening.getCaseInventory("casebuy").openInventory(paramPlayer);
        }
    }

    public static void checkCase(Player p, InventoryClickEvent clickEvent, String uuid, String caseType, String caseId, int amount) {
        if (clickEvent.isRightClick()) {
            CaseLootInventory.cases.put(uuid, caseType);
            CaseOpening.getCaseInventory("caseloot").openInventory(p);
        } else if (clickEvent.isLeftClick()) {
            if (amount < 1) {
                p.sendMessage(CaseOpening.prefix + "Du hast leider keine Cases mehr.");
                p.closeInventory();
                return;
            }
            Inventory inv = CaseOpening.getCaseInventory("loot").getInventory(p);
            new CaseAnimation(caseType, p, inv).start();
